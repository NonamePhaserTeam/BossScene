...
export default class Enemy extends Phaser.Physics.Arcade.Sprite {
  ...
  OnGuard(playerx: number, playery: number) {
    ...
    if (distanzaY_dal_player > -100) {
      // quando sta a terra
      if (
        distanzaX_dal_player < this.distanza_minima &&
        distanzaX_dal_player > 0
      ) {
        // follow right
      ...
      (distanzaY_dal_player > -150 &&
        distanzaX_dal_player < this.distanza_minima - 300 &&
        distanzaX_dal_player > 30) ||
      (distanzaX_dal_player > -this.distanza_minima + 300 &&
        distanzaX_dal_player < 30)
    ) {
